...
import com.wfp.lmmis.masterdata.dao.FactoryDaoImpl;
 ...
 */
@Controller
public class BKMEARepresentativeVerificationController
{
     ...
     * @return
     */
    @RequestMapping(value = "/bkmeaRepresentativeVerification", method = RequestMethod.GET)
    public String getAuditingPage(Model model, HttpServletRequest request, HttpSession session)
    {
        ...
        return "bkmeaRepresentativeVerification";
    }

    @RequestMapping(value = "/bkmeaRepresentativeVerification/list", method = RequestMethod.POST)
    public @ResponseBody
    void paginationUnionVerification(HttpServletRequest request, HttpServletResponse response)
    {
        //Fetch the page number from client
        System.out.println("in bkmeaRepresentativeVerification pagination controller");
        ...
        parameter.put("regularUser", 0);// as this is for regular users of lma
        parameter.put("bgmeaUser", 0);
        parameter.put("bkmeaUser", 1);
...
//                ja.put((applicant.getFirstNameInBangla() == null ? ' ' : applicant.getFirstNameInBangla()) + " " + (applicant.getMiddleNameInBangla() == null ? ' ' : applicant.getMiddleNameInBangla()) + " " + (applicant.getLastNameInBangla() == null ? ' ' : applicant.getLastNameInBangla()));
            ...
            System.out.println("fac "+applicant.getFactoryId());
            if ("en".equals(locale.getLanguage()))
            {
    ...
    @RequestMapping(value = "/bkmeaRepresentativeVerification", method = RequestMethod.POST)
            ...
            HttpServletRequest request, RedirectAttributes redirectAttributes)
    {
        ...
        parameter.put("beforeApplicationStatus", ApplicationStatus.BKMEA_REPRESENTITIVE_LEVEL_VERIFICATION);
        parameter.put("afterApplicationStatus", ApplicationStatus.DWA_OFFICER_LEVEL_VERIFICATION);
        String selectedApplicantList = applicantService.updateApplicantStatus(parameter);
        ControllerMessage controllerMessage = new ControllerMessage(ControllerMessageType.SUCCESS, localizer.getLocalizedText("approveDwaOfficerVerification", LocaleContextHolder.getLocale()));
        ...
        return "redirect:/bkmeaRepresentativeVerification";
    }

    @RequestMapping(value = "/bkmeaRepresentativeVerificationReport", method = RequestMethod.POST)
    ...
    private DynamicReport getApplicantDetailDynamicReport(ApplicantReportData applicantReportData) throws ColumnBuilderException, ClassNotFoundException
    {
        try
        {
            ...
            String beneficiaryReport = localizer.getLocalizedText("label.bkmeaRepresentativeApproval", locale);
