...
<script>
    $(function () {
        ...
        {
            $(menuSelect("${pageContext.request.contextPath}/payroll/union/create"));
        } else
        {
            $(menuSelect("${pageContext.request.contextPath}/payroll/municipal/create"));
        }
        var urlLang = "";
...
//                                $(this).find("td").eq(1).html(getNumberInBangla($(this).find("td").eq(1).html()));
    ...
    function loadParentPaymentCycle(fiscalYearId, paymentCycleSelectId, isActive) {
        ...
        if (isActive === true)
        {
            if ('${paymentGenerationForm.searchParameterForm.payrollListType.displayName}' === 'Union')
            {
                ajaxUrl = contextPath + "/getParentPaymentCycleIoList/" + fiscalYearId + "/1";
            } else
            {
...
<form:form id="paymentGenerationForm" action="${actionUrl}" method="post" class="form-horizontal" role="form" modelAttribute="paymentGenerationForm">
    <section class="content-header clearfix">
        ...
        <div class="pull-right" id="right_buttons">
            <button type="button" id="buttonSearchUnion" class="btn bg-gray"><span class="glyphicon glyphicon-search">&nbsp;</span><spring:message code="button.beneficiaryNumber" /></button>
