...
//                $("#right_buttons").hide();
        ...
        if ('${beneficiaryList}' != '') {
            ...
            $("#divBeneficiaryWoPaymentInfo").css("display", "block");
        }
...
//                var serializedData = $("#paymentGenerationForm").serialize();
//                console.log(serializedData);
//                $('#paymentGenerationTable').DataTable().destroy();
//                $('#paymentGenerationTable').DataTable({
//                    "processing": true,
//                    "pageLength": 10,
//                    "lengthMenu": [5, 10, 15, 20],
//                    "serverSide": true,
...
//                    },
//                    "ajax": {
//                        "url": path + "/paymentGeneration/beforeCreate/list",
//                        "type": "POST",
//                        "data": {
//                            "fiscalYear": $("#fiscalYear").val(),
//                            "paymentCycle": $("#paymentCycle").val(),
...
//                        if (selectedLocale === 'bn')
//                        {
//                            localizeBanglaInDatatable("paymentGenerationTable");
//                            $('#paymentGenerationTable  tr').not(":first").each(function () {
...
//                        }
//
//                        if ($('#paymentGenerationTable  tr').eq(1).find("td").get(3))
//                        {
//                            $("#right_buttons").show();
//                        }
//                    }
...
//            console.log('pc = ' + ${paymentGenerationForm.searchParameterForm.paymentCycle.id});
            loadChildPaymentCycle(fiscalYearId, paymentCycleSelectId, true);
            if ('${paymentGenerationForm.searchParameterForm.paymentCycle.id}' != '')
    ...
    function loadChildPaymentCycle(fiscalYearId, paymentCycleSelectId, isActive) {
...
<c:choose>
    <c:when test="${actionType eq 'create'}">
        <c:set var="actionUrl" value="${contextPath}/childPaymentGeneration/create" />
    </c:when>
    <c:when test="${actionType eq 'edit'}">
        <c:set var="actionUrl" value="${contextPath}/childPaymentGeneration/edit" />
    </c:when>
    <c:otherwise>
        <c:set var="actionUrl" value="${contextPath}/childPaymentGeneration/submit/${paymentGenerationForm.payrollSummary.id}" />
    </c:otherwise>
</c:choose>
...
<form:form id="paymentGenerationForm" action="${actionUrl}" method="post" class="form-horizontal" role="form" modelAttribute="paymentGenerationForm">
    <section class="content-header clearfix">
        <h1 class="pull-left">
            <spring:message code="label.payment" />&nbsp;<spring:message code="generation" />
            <small><i class="fa fa-arrow-circle-left"></i><a href="${contextPath}/payrollSummary/list"><spring:message code="label.backToList" /></a></small>
        </h1>
            ...
            <div class="col-xs-12">
                ...
                <div class="box">
                    <div class="box-body">
                        ...
                        <!--                        <table id="paymentGenerationTable" class="table table-bordered table-hover">
                                                    <thead>
                                                        <tr>
                                        ...
                                        <tr>
                                            <c:choose>
                                                <c:when test="${sessionScope.userDetail.schemeShortName == 'MA'}">
                                                    ...
                                                    <c:otherwise>
                                                    <th><spring:message code="label.municipalOrCityCorporation"/></th>
                                                    </c:otherwise>
                                                </c:choose>
                        ...
                        <div id="divBeneficiaryWoPaymentInfo" style="display: none">                            
                            <table id="beneficiaryListTable" class="table table-bordered table-hover">
                                ...
                                <tbody>
                                    <c:forEach var="beneficiary" items="${beneficiaryList}" varStatus="index">
                                        <tr>
                                            <td>
                                                <c:choose>
                                                    <c:when test="${selectedLocale eq 'en'}">
                                                        <c:out value="${beneficiary.fullNameInEnglish}"></c:out>
                                                    </c:when>
                                                    <c:otherwise>
                                                        <c:out value="${beneficiary.fullNameInBangla}"></c:out>
                                                    </c:otherwise>
                                                </c:choose>                                    
                                            </td>
                                            <td>                                        
                                                <c:if test="${selectedLocale eq 'en'}">
                                                    <c:out value="${beneficiary.nid}"></c:out>
                                                </c:if>
                                                <c:if test="${selectedLocale eq 'bn'}">                                            
                                                    <script>
                                                        var bnText = getNumberInBangla('${beneficiary.nid}');
                                                        document.writeln(bnText);
                                                    </script>
                                                </c:if>
                                            </td>  
                                            <td>
                                                <span id="dateOfBirth${index.index}"><fmt:formatDate pattern="dd-MM-yyyy" value="${beneficiary.dateOfBirth.time}" /></span>
                                                <c:if test="${selectedLocale eq 'en'}">

                                                </c:if>
                                                <c:if test="${selectedLocale eq 'bn'}">                                                    
                                                    <script>
                                                        $("#dateOfBirth${index.index}").text(getNumberInBangla($("#dateOfBirth${index.index}").text()));
                                                    </script>
                                                </c:if>
                                            </td>
                                            <td>                                            
                                                <c:if test="${selectedLocale eq 'en'}">
                                                    <c:out value="${beneficiary.mobileNo}"></c:out>
                                                </c:if>
                                                <c:if test="${selectedLocale eq 'bn'}">
                                                    <script>
                                                        if ('${beneficiary.mobileNo}' != '') {
                                                            var bnText = getNumberInBangla('' + ${beneficiary.mobileNo} + '');
                                                            document.writeln(bnText);
                                                        }
                                                    </script>
                                                </c:if>
                                            ...
                                            <td>
                                                ...
                                                <a href="${contextPath}/beneficiary/beneficiaryForm/${beneficiary.id}" title="${edit}"><span class="glyphicon glyphicon-edit"></span></a>
                                            </td>
                                        </tr>
                                    </c:forEach>
                                </tbody>
