...
//        });
    ...
    function getEligibility()
    {
        ...
        if ("${type}" == "bgmea")
        {
            url = "/checkBGMEAEligibility/" + $("#bgmeaFactoryId").val();
        } else
        {
            url = "/checkBKMEAEligibility/" + $("#bkmeaFactoryId").val();
        }
        $.ajax({
            url: contextPath + url,
            type: "POST",
    ...
    function askApproveSubmit()
    {
        var approvalMessage;
        if (selectedLocale === 'bn') {
            approvalMessage = "আপনি কি আবেদনকারীকে সুপারিশ করতে চান?";
        } else
        {
            approvalMessage = "Are you sure you want to recommend the applicant(s)?";
        }
        bootbox.confirm({
    ...
    function askSubmit()
    {
        ...
        {
            if (selectedLocale === 'bn') {
                approvalMessage = "আবেদনকারী(দের)কে বিজিএমইএ প্রতিনিধি  কতৃক  যাচাইয়ের জন্য পাঠাতে চান?";
            } else
            {
                approvalMessage = "Are you sure you want to Forward The Applicant(s) For BGMEA Representive Verification?";
            }
        } else
        {
            if (selectedLocale === 'bn') {
                approvalMessage = "আবেদনকারী(দের)কে বিকেএমইএ প্রতিনিধি  কতৃক যাচাইয়ের জন্য পাঠাতে চান?";
            } else
            {
                approvalMessage = "Are you sure you want to Forward The Applicant(s) For BKMEA Representative Verification?";
            }
        }
        bootbox.confirm({
            message: approvalMessage,
...
<c:set var="actionUrl" value="${contextPath}/recommendation" />
<section class="content-header clearfix">
    <h1 class="pull-left">
        <spring:message code="label.recommendation" />
    </h1>   
</section>
                    ...
                    <table id="applicantListTable" class="table table-bordered table-hover" cellspacing="0" width="100%">
                        <thead>
                            <tr>
                                ...
                                <th><spring:message code="label.imlmaRecommendationStatus"/></th>
        ...
        <div class="row">
            <div class="col-md-6" id="reasonSubmission">
                ...
                <div class="col-md-10">
                    ...
                    <br>
                    <button class="btn bg-blue" type="button" id="buttonAccept" name="action" value="accept" onclick="return askApproveSubmit();"><spring:message code="button.recommend" /></button>
            ...
            <div class="col-md-6" id="advanceBlock" style="display: none">
                <div>
                    <button class="btn bg-green" type="button" id="buttonAdvance" name="action" value="advance" onclick="return askSubmit();">
                        <c:if test="${type eq 'bgmea'}"><spring:message code="button.sendToBGMEAVerification" /></button></c:if>
                    <c:if test="${type eq 'bkmea'}"><spring:message code="button.sendToBKMEAVerification" /></button></c:if>
