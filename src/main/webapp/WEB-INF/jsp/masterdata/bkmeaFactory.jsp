<%-- 
    Document   : bkmeaFactory
    Created on : Feb 18, 2018, 5:43:05 PM
    Author     : user
--%>
...
<script>
    $(function () {
        $(menuSelect("${pageContext.request.contextPath}/factory/bkmea/list"));
...
<c:choose>
    <c:when test="${actionType eq 'create'}">
        <c:set var="actionUrl" value="${contextPath}/factory/bkmea/create" />
    </c:when>
    <c:otherwise>
        <c:set var="actionUrl" value="${contextPath}/factory/bkmea/edit/${factory.id}" />
    </c:otherwise>
</c:choose>
...
<form:form id="factoryForm" action="${actionUrl}" method="post" class="form-horizontal" role="form" modelAttribute="factory">
    <section class="content-header clearfix">
        <h1 class="pull-left">
            <spring:message code="label.bkmeaFactory" />&nbsp;<spring:message code="label.management" />
            <small><i class="fa fa-arrow-circle-left"></i><a href="${contextPath}/factory/bkmea/list"><spring:message code="label.backToList" /></a></small>
        </h1>
