<%-- 
    Document   : municipalOrCityCorporationList
    Created on : Jan 29, 2018, 1:18:08 PM
    Author     : user
--%>
...
<script>
    $(document).ready(function () {
        ...
        $("#municipalOrCityCorporationListForm").validate({
        ...
        {
            loadUnion('${searchParameterForm.upazila.id}', $('#municipalOrCityCorporationId'));
        }

        $("#buttonSearch").click(function ()
        {
            $("#municipalOrCityCorporationListForm").validate();
            if ($("#municipalOrCityCorporationListForm").valid())
            {
                $('#municipalOrCityCorporationListTable').DataTable().destroy();
                $('#municipalOrCityCorporationListTable').DataTable({
                    "processing": true,
                    "pageLength": 10,
                    ...
                    "ajax": {
                        "url": path + "/municipalOrCityCorporation/list",
                        "type": "POST",
                        "data": {
                            "divisionId": $('#divisionId').val(),
                            "districtId": $('#districtId').val(),
                            "upazilaId": $('#upazilaId').val(),
                            "coverageAreaId": $('#coverageAreaId').val()
                        }
                    },
                    ...
                    "fnDrawCallback": function (oSettings) {
                        ...
                        {
                            localizeBanglaInDatatable("municipalOrCityCorporationListTable");
                        }
                    }
                });
            }
    ...
    function loadPresentDistrictList(selectObject) {
        ...
        if (divId !== '') {
            ...
            resetSelect($('#municipalOrCityCorporationId'));
        }
    }
    ...
    function loadPresentUnionList(selectObject) {
        ...
        var municipalOrCityCorporationSelectId = $('#municipalOrCityCorporationId');

        if (upazilaId !== '') {
            loadUnion(upazilaId, municipalOrCityCorporationSelectId);
        } else {
            resetSelect(municipalOrCityCorporationSelectId);
        }
    }
</script>
<c:set var="contextPath" value="${pageContext.request.contextPath}" />
<c:set var="actionUrl" value="" />
<form:form id="municipalOrCityCorporationListForm" action="${actionUrl}" method="post" class="form-horizontal" role="form" modelAttribute="searchParameterForm">
    <section class="content-header clearfix">
        <h1 class="pull-left">
            <spring:message code="label.municipalOrCityCorporation" />&nbsp;<spring:message code="label.list" />
            <small></small>
        </h1>
        <div class="pull-right">
            <a href="${contextPath}/municipalOrCityCorporation/create" class="btn bg-blue">
    ...
    <section class="content">
        <div class="form-group">
            <div class="col-md-4">
                ...
                <div class="form-group">
                    <label for="middleNameInput" class="col-md-4 control-label"><spring:message code="label.coverageArea" /></label>
                    <div class="col-md-8">
                        <spring:message code='label.select' var="select"/>
                        <form:select class="form-control" path="coverageArea" id="coverageAreaId">
                            <form:option value="" label="${select}"></form:option>
                            <c:choose>
                                <c:when test="${pageContext.response.locale eq 'bn'}">
                                    <c:forEach items="${coverageAreaList}" var="coverageArea">
                                        <form:option value="${coverageArea}" label="${coverageArea.displayNameBn}"></form:option> 
                                    </c:forEach>
                                </c:when>
                                <c:otherwise>
                                    <c:forEach items="${coverageAreaList}" var="coverageArea">
                                        <form:option value="${coverageArea}" label="${coverageArea.displayName}"></form:option> 
                                    </c:forEach>
                                </c:otherwise>
                            </c:choose>

                        </form:select>
                        <form:errors path="coverageArea" cssStyle="color:red"></form:errors>
                        </div>
                    </div>
                    <div class="form-group">
            ...
            <div class="col-xs-12">
                <div class="box">
                    <div class="box-body">
                        <table id="municipalOrCityCorporationListTable" class="table table-bordered table-hover">
                            <thead>
                                <tr>
