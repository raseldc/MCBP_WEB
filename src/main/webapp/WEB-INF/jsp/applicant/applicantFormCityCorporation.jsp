...
//    var upazilaList = [114, 662, 8134, 8527, 9141, 395, 7263];
    ...
    $(function () {
        ...
        {
            $(menuSelect("${pageContext.request.contextPath}/applicant/cityCorporation/create"));
        } else
        {
            $(menuSelect("${pageContext.request.contextPath}/applicant/cityCorporation/list"));
        }
...
//                    max: 4
        ...
        {
            loadDistrictFromUpazilaTable('${applicantForm.presentDistrict.id}', $('#presentUpazila'));
        } else if ('${applicantForm.isUnionAvailable}' === 'false' && '${applicantForm.presentUpazila.id}' !== "")
        {
            loadCityCorporation('${applicantForm.presentUpazila.id}', $('#presentUnion'));
        }
...
//                        isExistOtherMIS('<spring:message code="label.failure" />', '<spring:message code="label.ok" />');
    ...
    function loadPermUpazilaList(selectObject) {
        ...
        if (distId !== '') {
            loadDistrictFromUpazilaTable(distId, upazillaSelectId, '${applicantForm.permanentUpazila.id}');
        } else {
    ...
    function loadPresentUpazilaList(selectObject) {
        ...
        if (distId !== '') {
            loadDistrictFromUpazilaTable(distId, upazillaSelectId, '${applicantForm.presentUpazila.id}');
        } else {
            resetSelect(upazillaSelectId);
            resetSelect($('#presentUnion'));
        }
    ...
    function loadPermUnionList(selectObject) {
        ...
        if (upazilaId !== '') {
            loadCityCorporation(upazilaId, unionSelectId, '${applicantForm.permanentUnion.id}');
        } else {
...
//        }
        ...
        if (upazilaId !== '') {
            loadCityCorporation(upazilaId, unionSelectId, '${applicantForm.presentUnion.id}');
        } else {
            resetSelect(unionSelectId);
        }
    }
...
<form:form id="applicantInfoForm" action="${actionUrl}" class="form-horizontal" enctype="multipart/form-data" role="form" modelAttribute="applicantForm">
    <section class="content-header clearfix">
        <h1 class="pull-left">
            ...
            <small><i class="fa fa-arrow-circle-left"></i><a href="${contextPath}/applicant/cityCorporation/list"><spring:message code="label.backToList"/></a></small>
        </h1>
