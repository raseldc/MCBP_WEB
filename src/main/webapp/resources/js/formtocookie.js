...
function loadDDL(ddl, value) {    
    var option = ddl.find("option[value='" + value + "']"); 
    if (option.length < 0) {
        setTimeout(function () {
            loadDDL(ddl, value);
        }, 500);
...
function loadFormFromCookie(form) {
    var name = $(form).attr('id');
    var formValues = Cookies.getJSON(name);
    $.each(formValues, function (e) {
        var inputKey = e;
        console.log(e);
        e = $("[name='" + e + "'");
        if (e.prop('tagName').toLowerCase() === 'select') {
            var value = formValues[inputKey];
            if(value !== "")
            {
                loadDDL(e, formValues[inputKey]);                                     
            }
        } else {
            e.val(formValues[inputKey]);
        }
    });
}
//
